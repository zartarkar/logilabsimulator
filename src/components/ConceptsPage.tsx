...
function Section({ id, title, children }: { id: string; title: string; children: ReactNode }) {
  return (
    ...
    >
      <h2 className="mb-6 flex items-center gap-3 font-display text-xl font-bold sm:text-2xl">
        {title}
      </h2>
      <div className="space-y-8 text-sm leading-8 sm:text-base sm:leading-9">{children}</div>
    </section>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl space-y-7 py-7 sm:space-y-9 sm:py-10">
        ...
        <Section
          id="signals"
          title={say("01 / Boolean Operations & Logic Gates", "০1 / বুলিয়ান অপারেশন ও লজিক গেট")}
        >
          <p>
            {say(
        ...
        <Section id="tables" title={say("02 / Truth Tables", "০২ / সত্যক সারণি")}>
          ...
          <Table
            headers={["A", "B", ...gates]}
            rows={inputRows(2).map(([x, y]) => [x!, y!, ...gates.map((g) => gateValue(g, x!, y!))])}
            active={a * 2 + b}
          />
        ...
        <Section id="morgan" title={say("04 / De Morgan’s Theorems", "০৪ / ডি মর্গ্যানের উপপাদ্য")}>
          <p>
            {say(
