...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Check, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, Lightbulb, MapPin, ToggleLeft, Trash2, Sparkles, Maximize2, PartyPopper, RotateCcw } from "lucide-react";
...
function nodeExpression(id: string, nodes: SBNode[], edges: Edge[], memo = new Map<string, string>()): string {
  ...
  const joined = inputs.join(node.kind === "OR" || node.kind === "NOR" ? " + " : node.kind === "XOR" || node.kind === "XNOR" ? " ⊕ " : " · ");
  const notInput = inputs[0] ?? "?";
  const expression = node.kind === "NOT"
    ? `${/[+·⊕]/.test(notInput) ? `(${notInput})` : notInput}'`
    : node.kind === "OUTPUT" ? (inputs[0] ?? "?") : joined || node.kind;
...
export function diagnosePracticeCircuit(nodes: SBNode[], edges: Edge[], expectedExpression: string, bn: boolean): string[] {
  const errors: string[] = [];
  ...
  const describeGate = (node: SBNode) => {
    const xPosition = node.x <= minX + (maxX - minX) / 2
      ? (bn ? "বাম পাশের" : "left-side")
      : (bn ? "ডান পাশের" : "right-side");
    const yRatio = (node.y - minY) / Math.max(maxY - minY, 1);
    const yPosition = yRatio < 0.34
      ? (bn ? "উপরের" : "upper")
      : yRatio > 0.66
        ? (bn ? "নিচের" : "lower")
        : (bn ? "মাঝের" : "middle");
    return bn ? `canvas-এর ${yPosition} ${xPosition} ${node.kind} gate` : `the ${yPosition} ${xPosition} ${node.kind} gate on the canvas`;
  };
  ...
  if (!nodes.length) return [bn ? "Canvas খালি। প্রথমে প্রয়োজনীয় input, gate এবং output LED যোগ করো।" : "The canvas is empty. Add the required inputs, gates, and output LED first."];
  if (inputs.length < expectedVariables.length) errors.push(bn ? `${expectedVariables.length - inputs.length}টি input switch কম আছে।` : `${expectedVariables.length - inputs.length} input switch(es) are missing.`);
  if (!outputs.length) errors.push(bn ? "Output LED যোগ করা হয়নি।" : "The output LED is missing.");
  if (outputs.length > 1) errors.push(bn ? "একটির বেশি Output LED আছে। একটি final output রাখো।" : "There is more than one output LED. Keep one final output.");
  ...
  if (missingVariables.length) errors.push(bn ? `এই inputগুলো পাওয়া যায়নি: ${missingVariables.join(", ")}।` : `These inputs are missing: ${missingVariables.join(", ")}.`);

  nodes.filter((node) => node.kind !== "INPUT").forEach((node) => {
    ...
    if (incoming.length < required) {
      ...
      errors.push(bn ? `${name}-এ ${required - incoming.length}টি সংযোগ কম আছে।` : `${name} is missing ${required - incoming.length} input connection(s).`);
    }
  });

  const invalidEdges = edges.filter((edge) => !nodes.some((node) => node.id === edge.source) || !nodes.some((node) => node.id === edge.target));
  if (invalidEdges.length) errors.push(bn ? "এক বা একাধিক তার ভাঙা component-এর সঙ্গে যুক্ত। সেগুলো মুছে আবার সংযোগ দাও।" : "One or more wires point to a missing component. Remove and reconnect them.");
  ...
  if (nodes.some((node) => hasCycle(node.id))) errors.push(bn ? "Circuit-এ একটি loop তৈরি হয়েছে। যে তারটি আগের gate-এ ফিরে গেছে সেটি সরাও।" : "The circuit contains a loop. Remove the wire that feeds back into an earlier gate.");
  ...
  if (outputs[0]) {
    ...
    visit(outputs[0].id);
    const disconnected = nodes.filter((node) => node.kind !== "OUTPUT" && !connectedToOutput.has(node.id));
    if (disconnected.length) errors.push(bn ? `${disconnected.length}টি component final output-এর সঙ্গে যুক্ত নয়।` : `${disconnected.length} component(s) are not connected to the final output.`);
  }

  if (!checkCircuitMatchesChallengeTarget(nodes, edges, expectedExpression) && outputs[0]) {
    const outputInput = outputs[0] ? edges.find((edge) => edge.target === outputs[0].id) : undefined;
    const actualFinalGate = outputInput ? nodes.find((node) => node.id === outputInput.source) : undefined;
    ...
    if (expectedFinalGate && actualFinalGate && actualFinalGate.kind !== expectedFinalGate) {
      ...
      const mismatch = expectedAst ? findFirstGateMismatch(nodes, edges, expectedAst, actualFinalGate.id) : undefined;
...
function findFirstGateMismatch(
  ...
  if (expected.kind === "VAR") return actual.kind === "INPUT" && actual.label === expected.name
    ? undefined
    : { id: actual.id, expected: `input ${expected.name}`, actual: actual.kind === "INPUT" ? `input ${actual.label}` : actual.kind, branch: expected.expr };
  if (expected.kind === "CONST") return undefined;
  if (actual.kind !== expected.kind) return { id: actual.id, expected: expected.kind, actual: actual.kind, branch: expected.expr };

  const incomingIds = (id: string) => edges
    .filter((edge) => edge.target === id)
    .sort((a, b) => Number((a.targetHandle ?? "in-0").split("-")[1]) - Number((b.targetHandle ?? "in-0").split("-")[1]))
  ...
  if (associative) {
    ...
    while (actualChildren.length < expected.children.length) {
      const expandableIndex = actualChildren.findIndex((id) => nodes.find((node) => node.id === id)?.kind === expected.kind);
    ...
    if (actualChildren.length < expected.children.length) {
      const labelsFromExpected = (node: AstNode): string => [...new Set(
        node.kind === "VAR" ? [node.name] : node.kind === "CONST" ? [] : node.children.flatMap((child) => labelsFromExpected(child).split(",").filter(Boolean)),
      )].sort().join(",");
      const labelsFromActual = (id: string, seen = new Set<string>()): string => {
        ...
        return [...new Set(incomingIds(id).flatMap((source) => labelsFromActual(source, next).split(",").filter(Boolean)))].sort().join(",");
      };
      ...
      const wrongChainGate = actualChildren
        ...
        .find((node) => node
          ...
          && (
            ...
            || incomingIds(node.id).filter((source) => expectedChildLabels.has(labelsFromActual(source))).length >= 2
          ));
      if (wrongChainGate) {
        return { id: wrongChainGate.id, expected: expected.kind, actual: wrongChainGate.kind, branch: expected.expr };
      }
    }
  }
  const variables = [...new Set(nodes.filter((node) => node.kind === "INPUT").map((node) => node.label))].sort();
  const envs = [...enumerateEnvs(variables)];
  const actualValue = (id: string, env: Record<string, 0 | 1>, visiting = new Set<string>()): 0 | 1 => {
    ...
    return node.kind === "OUTPUT" ? (values[0] ?? 0) : values.length ? evalGate(node.kind, values) : 0;
  };
  const expectedVariables = (node: AstNode): Set<string> => node.kind === "VAR"
    ? new Set([node.name])
    : node.kind === "CONST"
      ? new Set()
  ...
  const distance = (expectedChild: AstNode, id: string) => {
    ...
    const variableDifference = [...expectedVars].filter((name) => !actualVars.has(name)).length
      + [...actualVars].filter((name) => !expectedVars.has(name)).length;
    const truthDifference = envs.reduce((total, env) => (
      total + Number(evaluateAst(expectedChild, env) !== actualValue(id, env))
    ), 0);
  ...
  for (let index = 0; index < expected.children.length; index++) {
    ...
    if (!match) return { id: actual.id, expected: `branch ${expectedChild.expr}`, actual: "missing connection", branch: expectedChild.expr };
  ...
  if (extra) {
    ...
    return { id: extra, expected: "no extra branch", actual: extraNode?.kind ?? "extra connection", branch: expected.expr };
  }
  return undefined;
}
...
function getChallengeLearningOutcomes(challengeId: string, bn: boolean): string[] {
  const outcomes: Record<string, { bn: string[]; en: string[] }> = {
    "and-gate": {
      bn: ["AND gate কখন output 1 দেয় তা বুঝেছ", "দুইটি input সঠিক gate port এ যুক্ত করতে শিখেছ", "Input বদলে circuit output যাচাই করতে পেরেছ"],
      en: ["Understood when an AND gate produces 1", "Connected two inputs to the correct gate ports", "Verified circuit output by changing input values"],
    },
    "or-gate": {
      bn: ["OR gate এর output rule বুঝেছ", "একাধিক input branch এক gate এ যুক্ত করতে শিখেছ", "Truth table অনুযায়ী output পরীক্ষা করতে পেরেছ"],
      en: ["Understood the OR gate output rule", "Connected multiple input branches to one gate", "Tested the output against the truth table"],
    },
    "xor-gate": {
      bn: ["দুই input ভিন্ন হলে XOR কেন 1 দেয় তা বুঝেছ", "XOR circuit সঠিকভাবে wire করতে শিখেছ", "সব input combination দিয়ে result যাচাই করেছ"],
      en: ["Understood why XOR is 1 when inputs differ", "Wired an XOR circuit correctly", "Verified the result across input combinations"],
    },
    "intermediate-1": {
      bn: ["AB এবং A'C নামে দুইটি product branch তৈরি করেছ", "A input কে NOT করে complemented signal ব্যবহার করেছ", "দুই branch OR করে final expression তৈরি করেছ"],
      en: ["Built the AB and A'C product branches", "Used a NOT gate to create the complemented A signal", "Combined both branches with OR to form the final expression"],
    },
    "intermediate-2": {
      bn: ["Bracket অনুযায়ী A+B অংশটি আগে তৈরি করেছ", "Intermediate OR output কে C এর সঙ্গে AND করেছ", "Operation order circuit structure এ প্রয়োগ করেছ"],
      en: ["Built the grouped A+B operation first", "ANDed the intermediate OR output with C", "Applied expression order to the circuit structure"],
    },
    "intermediate-3": {
      bn: ["AB, AC এবং BC নামে তিনটি parallel product branch তৈরি করেছ", "দুই input gate chain ব্যবহার করে তিন branch combine করেছ", "Multi branch expression এর final output যাচাই করেছ"],
      en: ["Built the parallel AB, AC, and BC product branches", "Combined three branches using two input gate chains", "Verified the final output of a multi branch expression"],
    },
    "intermediate-4": {
      bn: ["A+B এবং C+D নামে দুইটি sum group তৈরি করেছ", "দুই group এর output AND করে product of sums বানিয়েছ", "Bracket এবং final gate এর সম্পর্ক বুঝেছ"],
      en: ["Built the A+B and C+D sum groups", "ANDed both group outputs to create a product of sums", "Understood how brackets determine the final gate"],
    },
    "hard-1": {
      bn: ["A'BC, AB'C এবং ABC' branch আলাদাভাবে তৈরি করেছ", "প্রতিটি branch এ সঠিক variable complement করেছ", "তিনটি complex branch OR chain দিয়ে combine করেছ"],
      en: ["Built the A'BC, AB'C, and ABC' branches separately", "Complemented the correct variable in every branch", "Combined three complex branches through an OR chain"],
    },
    "hard-2": {
      bn: ["B কে complement করে inner A+B' group তৈরি করেছ", "পুরো grouped output এ আবার NOT প্রয়োগ করেছ", "Nested complement এর পরে C branch OR করেছ"],
      en: ["Complemented B inside the A+B' group", "Applied NOT again to the complete grouped output", "ORed the nested complement result with C"],
    },
    "hard-3": {
      bn: ["একই circuit এ XOR এবং NAND operation ব্যবহার করেছ", "দুই ধরনের gate branch স্বাধীনভাবে তৈরি করেছ", "Mixed gate outputs OR করে final result পেয়েছ"],
      en: ["Used XOR and NAND operations in one circuit", "Built two different gate branches independently", "ORed the mixed gate outputs to get the final result"],
    },
    "hard-4": {
      bn: ["A+B' এবং E+F' inner group সঠিকভাবে তৈরি করেছ", "Group দুটিকে যথাক্রমে C ও D এর সঙ্গে AND করেছ", "দুইটি multi stage branch OR করে final circuit শেষ করেছ"],
      en: ["Built the A+B' and E+F' inner groups correctly", "ANDed those groups with C and D respectively", "Completed the circuit by ORing two multi stage branches"],
    },
  };
  const selected = outcomes[challengeId];
  return selected?.[bn ? "bn" : "en"] ?? (bn
    ? ["Expression কে gate operation এ ভাগ করতে পেরেছ", "Gate ও wire সঠিকভাবে ব্যবহার করেছ", "Circuit output যাচাই করতে পেরেছ"]
    : ["Translated an expression into gate operations", "Used gates and wires correctly", "Verified the circuit output"]);
}

export function locatePracticeNodeErrors(nodes: SBNode[], edges: Edge[], expectedExpression: string, bn: boolean): Record<string, string> {
  const nodeErrors: Record<string, string> = {};
  const outputs = nodes.filter((node) => node.kind === "OUTPUT");
  nodes.filter((node) => node.kind !== "INPUT").forEach((node) => {
    const required = ARITY[node.kind] ?? 2;
    const missing = required - edges.filter((edge) => edge.target === node.id).length;
    if (missing > 0) nodeErrors[node.id] = bn ? `এখানে আরও ${missing}টি input wire দাও` : `Add ${missing} more input wire(s) here`;
  });
  if (outputs[0]) {
    ...
    visit(outputs[0].id);
    nodes.filter((node) => node.kind !== "OUTPUT" && !connected.has(node.id)).forEach((node) => {
      nodeErrors[node.id] ??= bn ? "এটি final output-এর সঙ্গে যুক্ত করো" : "Connect this to the final output";
    });
    if (!checkCircuitMatchesChallengeTarget(nodes, edges, expectedExpression)) {
      ...
      const expectedAst = getChallengeAst(expectedExpression);
      const mismatch = finalGate && expectedAst ? findFirstGateMismatch(nodes, edges, expectedAst, finalGate.id) : undefined;
      if (mismatch) {
        nodeErrors[mismatch.id] = formatPracticeMismatch(mismatch, bn);
      } else if (finalGate && !Object.keys(nodeErrors).length) {
        nodeErrors[finalGate.id] = bn ? "এই branch-এর আগের gate ও wire পরীক্ষা করো" : "Check the earlier gates and wires in this branch";
      }
    }
  }
  return nodeErrors;
...
function Inner({ isPracticeMode: initialPracticeMode }: { isPracticeMode: boolean }) {
  ...
  const [activeChallengeId, setActiveChallengeId] = useState<string>(PRACTICE_CHALLENGES[0]?.id ?? "");
  ...
  const difficultyChallenges = PRACTICE_CHALLENGES.filter((challenge) => challenge.difficulty === difficulty);
  const challengeLearningOutcomes = getChallengeLearningOutcomes(activeChallenge.id, lang === "bn");
  const activeGuide = !hasSelectedDifficulty || guideSkipped
    ? undefined
  ...
  const changeDifficulty = useCallback((nextDifficulty: "beginner" | "intermediate" | "hard") => {
    setHasSelectedDifficulty(true);
    setDifficulty(nextDifficulty);
    const firstChallenge = PRACTICE_CHALLENGES.find((challenge) => challenge.difficulty === nextDifficulty);
  ...
  const removeEdgeOnTap = useCallback((event: { stopPropagation: () => void }, edge: Edge) => {
  ...
  return (
    <div className={`flex h-full min-h-0 flex-col lg:flex-row bg-transparent sandbox-container overflow-hidden touch-none lg:touch-auto ${isPracticeMode ? "practice-builder" : ""}`}>
      <AlertDialog open={showChallengeSuccess} onOpenChange={setShowChallengeSuccess}>
        <AlertDialogContent className="max-w-lg overflow-hidden border-2 border-primary/35 bg-card p-0 text-center shadow-2xl sm:text-center">
          ...
          <div className="px-6 py-5 text-left">
            ...
            <ul className="mt-3 space-y-2.5">
              {challengeLearningOutcomes.map((outcome) => (
                <li key={outcome} className="flex items-start gap-3 rounded-xl border border-primary/15 bg-primary/[0.045] px-3 py-2.5 text-sm leading-relaxed text-foreground">
          ...
          <AlertDialogFooter className="border-t border-border bg-muted/30 px-6 py-4 sm:justify-center">
            <AlertDialogAction className="min-w-36 font-bold">{lang === "bn" ? "চালিয়ে যাও" : "Continue"}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      <AlertDialog open={challengeErrors.length > 0} onOpenChange={(open) => !open && setChallengeErrors([])}>
        <AlertDialogContent className="max-w-md border-destructive/30">
          ...
          <AlertDialogHeader className="text-center sm:text-center">
            <AlertDialogTitle>{lang === "bn" ? "আরও একটু ঠিক করতে হবে" : "A few things need fixing"}</AlertDialogTitle>
            <AlertDialogDescription>
              {lang === "bn" ? "নিচের সমস্যাগুলো ঠিক করে আবার Check করো।" : "Fix the issues below, then check your circuit again."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <ul className="space-y-2 rounded-xl bg-muted/60 p-3 text-sm">
            {challengeErrors.map((error, index) => (
              <li key={error} className="flex items-start gap-2"><span className="font-bold text-destructive">{index + 1}.</span><span>{error}</span></li>
            ))}
          </ul>
          <AlertDialogFooter className="sm:justify-center">
            <AlertDialogAction>{lang === "bn" ? "ঠিক আছে, ঠিক করব" : "Got it, I'll fix it"}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      <aside className="flex shrink-0 flex-col gap-2 border-b border-border bg-card/60 backdrop-blur-sm p-2.5 lg:w-64 lg:overflow-y-auto lg:border-b-0 lg:border-r z-10 touch-auto no-scrollbar">
        <div className="practice-panel order-1 relative w-full shrink-0 overflow-hidden rounded-xl border-2 border-primary/45 bg-gradient-to-br from-primary/15 via-card to-card p-3 shadow-md ring-1 ring-primary/10 flex flex-col gap-2.5 lg:sticky lg:top-0 lg:z-20">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {isPracticeMode && (
                <button className="text-[10px] font-medium text-muted-foreground underline-offset-2 hover:underline" onClick={() => {
            ...
            {!isPracticeMode ? (
              <div>
                <p className="mb-2 text-[11px] leading-relaxed text-muted-foreground">
                  {lang === "bn" ? "তৈরি expression দেখে circuit বানাও। Beginner level-এ ধাপে ধাপে guide পাওয়া যাবে।" : "Build circuits from ready-made expressions. Beginner level includes an interactive step-by-step guide."}
                </p>
                <Button
                  size="sm"
                ...
                >
                  <Sparkles className="mr-1 h-3.5 w-3.5" /> {lang === "bn" ? "চ্যালেঞ্জ শুরু করো" : "Start a challenge"}
                </Button>
              </div>
            ) : activeChallenge && (
              <div className="space-y-2">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">Difficulty</label>
                  <select
                    aria-label="Select practice difficulty"
                    value={hasSelectedDifficulty ? difficulty : ""}
                    onChange={(event) => changeDifficulty(event.target.value as "beginner" | "intermediate" | "hard")}
                  ...
                  >
                    <option value="" disabled>{lang === "bn" ? "লেভেল বেছে নাও" : "Choose a level"}</option>
                ...
                {hasSelectedDifficulty && <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">Expression</label>
                  <select
                ...
                {hasSelectedDifficulty && difficulty !== "beginner" && (
                  <Button
                    ...
                    className="h-8 w-full text-xs"
                    onClick={() => {
                      const errors = diagnosePracticeCircuit(nodes, edges, activeChallenge.target, lang === "bn");
                      if (errors.length) {
                        setChallengeNodeErrors(locatePracticeNodeErrors(nodes, edges, activeChallenge.target, lang === "bn"));
                        const incompleteNodeIds = nodes
                          .filter((node) => {
                            if (node.kind === "INPUT") return false;
                            const required = node.kind === "OUTPUT" || node.kind === "NOT" ? 1 : 2;
                            return edges.filter((edge) => edge.target === node.id).length < required;
                          })
                          .map((node) => node.id);
                        if (incompleteNodeIds.length) {
                          ...
                          const finalEdge = output ? edges.find((edge) => edge.target === output.id) : undefined;
                  ...
                  >
                    <Check className="mr-1 h-3.5 w-3.5" /> {lang === "bn" ? "সার্কিট যাচাই করো" : "Check circuit"}
                  </Button>
                )}
              </div>
            )}
        ...
        <div ref={paneRef} data-tour="builder-canvas" data-practice-target="canvas" className="flex-1 relative">
          ...
          {isMobile && isPracticeMode && selectedIds.length === 1 && (
            <div className="absolute bottom-2 right-2 z-30 flex items-center gap-0.5 rounded-lg border border-border bg-card/90 p-1 shadow-md backdrop-blur-sm" aria-label="Move selected component">
              ...
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveSelectedNode(0, -28)} aria-label="Move up">
                <ArrowUp className="h-3 w-3" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveSelectedNode(0, 28)} aria-label="Move down">
                <ArrowDown className="h-3 w-3" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveSelectedNode(28, 0)} aria-label="Move right">
          ...
          <section className="practice-guide shrink-0 border-t border-border bg-card/95 px-4 py-2.5 backdrop-blur-sm" aria-label="Guided practice step">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                <Button variant="outline" size="sm" className="h-8 text-xs" disabled={guideIndex === 0} onClick={() => setGuideIndex((idx) => Math.max(0, idx - 1))}>
                  <ChevronLeft className="mr-1 h-3 w-3" aria-hidden="true" /> Previous
                </Button>
                <Button variant="default" size="sm" className="h-8 text-xs" disabled={guideIndex === activeChallenge.guide.length - 1 || !completedSteps.has(guideIndex)} onClick={() => setGuideIndex((idx) => Math.min(activeChallenge.guide.length - 1, idx + 1))}>
                  Next <ChevronRight className="ml-1 h-3 w-3" aria-hidden="true" />
                </Button>
                <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => loadChallenge(activeChallenge.id)}>
                ...
                <Button
                  ...
                  onClick={() => {
                    setGuideSkipped(true);
                    toast.info(lang === "bn" ? "গাইড বাদ দেওয়া হয়েছে। এখন নিজের মতো সার্কিট বানাও।" : "Guide skipped. You can now build the circuit on your own.");
                  }}
                >
