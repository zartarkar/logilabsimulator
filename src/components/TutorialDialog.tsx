...
type TourTab = "simulator" | "builder";

export interface TutorialHandle {
  startAutomatically: (finishTab: "simulator") => void;
}
...
export const TutorialDialog = forwardRef<TutorialHandle, { className?: string; onSelectTab: (tab: TourTab) => void }>(function TutorialDialog({ className = "", onSelectTab }, ref) {
  ...
  const startTour = useCallback((finishTab?: "simulator") => {
    ...
    window.setTimeout(() => {
      ...
      const steps: DriveStep[] = [
        ...
        { element: '[data-tour="navigation"]', popover: { title: bn ? "সার্কিট নিয়ে কাজের দুটি জায়গা" : "Two circuit workspaces", description: bn ? "সিমুলেটরে রাশি থেকে গেট তৈরি করো। তারপর বিল্ডারে নিজের হাতে সার্কিট তৈরি ও অনুশীলন করো।" : "Use the simulator to turn expressions into gates, then build and practise your own circuits in the builder.", side: "bottom", align: "center" } },
        ...
        { element: '[data-tour="expression-editor"]', popover: { title: bn ? "১. বুলিয়ান রাশি লিখো" : "1. Enter a Boolean expression", description: bn ? "নিজের রাশি লিখতে পারো অথবা নিচের প্রস্তুত উদাহরণ থেকে বেছে নিতে পারো। বন্ধনী ও পরিপূরক চিহ্নও ব্যবহার করা যাবে।" : "Enter your own expression or choose a prepared example. Parentheses and complement notation are supported.", side: "right", align: "start" } },
        ...
        { element: '[data-tour="simulator-canvas"]', popover: { title: bn ? "সার্কিটের ফল সরাসরি দেখো" : "Watch the circuit respond", description: bn ? "ইনপুট পরিবর্তন করে সক্রিয় তার অনুসরণ করো। মোবাইলে ক্যানভাস স্থির থাকবে। পুরো সার্কিট দেখতে Fit চাপো।" : "Change inputs and follow active wires. The mobile canvas stays fixed; use Fit to see the whole circuit.", side: "left", align: "center" } },
        { element: '.nav-tab-builder', popover: { title: bn ? "এবার নিজে সার্কিট বানাই" : "Now build one yourself", description: bn ? "পরবর্তী ধাপে গাইড সার্কিট নির্মাণ ও অনুশীলনের অংশ খুলবে।" : "The next step opens the circuit builder and practice workspace.", side: "bottom", align: "center", onNextClick: goTo("builder") } },

        { element: '[data-tour="practice-panel"]', popover: { title: bn ? "২. লক্ষ্যভিত্তিক অনুশীলন" : "2. Goal-based practice", description: bn ? "চ্যালেঞ্জ চালু করে সহজ, মধ্যম বা কঠিন স্তর বেছে নাও। সহজ স্তরে ধাপে ধাপে সহায়তা থাকবে এবং অন্য স্তরে নিজের সার্কিট যাচাই করা যাবে।" : "Start a challenge and choose beginner, intermediate, or hard. Beginner includes guided steps; the other levels let you check your own solution.", side: "right", align: "start" } },
        ...
        { element: '[data-tour="builder-canvas"]', popover: { title: bn ? "তার দিয়ে সার্কিট সম্পূর্ণ করো" : "Wire the circuit", description: bn ? "প্রথমে ডান পাশের OUT বিন্দুতে, তারপর পরের গেটের বাম পাশের IN বিন্দুতে চাপো। টেনেও তার জোড়া যায়। ভুল তার একবার চাপলেই মুছে যাবে। ফাঁকা জায়গা টেনে পুরো সার্কিট সরাতে পারবে।" : "Tap an OUT dot, then an IN dot to connect them, or drag between the dots. Tap an incorrect wire to remove it, and drag empty space to pan.", side: "left", align: "center" } },
  ...
  const startAutomatically = useCallback((finishTab: "simulator") => {
    let timer: number;
